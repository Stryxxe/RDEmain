  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-lg transition-all duration-300 ease-in-out ${
        isFullscreen ? 'fixed inset-0 z-50 rounded-none overflow-hidden' : 'w-full'
      }`}
    >
      {/* Header Bar */}
      ...
      {/* PDF Content */}
      <div className={`bg-gray-50 ${isFullscreen ? 'h-full flex-1 overflow-hidden' : 'p-4'}`}>
        <div className={`bg-white border border-gray-200 rounded-lg overflow-hidden ${isFullscreen ? 'h-full' : ''}`}>
          {isLoading && (
            <div className={`flex flex-col items-center justify-center text-gray-500 text-center px-4 ${isFullscreen ? 'h-full' : 'min-h-[400px]'}`}>
          ...
          {pdfPath && !pdfError && !isLoading ? (
            <iframe
              ...
              style={!isFullscreen ? { minHeight: '600px', height: '600px' } : {}}
              title={title}
