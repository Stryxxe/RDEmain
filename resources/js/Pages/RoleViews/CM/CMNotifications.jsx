...
const CMNotifications = () => {
    const { user } = useAuth();
    const { refreshAllNotifications, isRefreshing: notificationRefreshing } =
        useNotifications();
    ...
    const fetchNotifications = async () => {
        try {
            ...
            const response = await (window.axios || axiosInstance).get(
                "/notifications",
                {
                    headers: {
    ...
    const markAsRead = async (notificationId) => {
        try {
            if (!user) return;
            await (window.axios || axiosInstance).put(
    ...
    const deleteNotification = async (notificationId) => {
        try {
            if (!user) return;
            await (window.axios || axiosInstance).delete(
