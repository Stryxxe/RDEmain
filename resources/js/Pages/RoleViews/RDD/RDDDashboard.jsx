  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (proposalsResponse.success) {
        ...
        const transformedProposals = proposalsResponse.data.map(proposal => {
          ...
          return {
            ...
            funding: `₱${Number(proposal.proposedBudget).toLocaleString()}`,
        ...
        console.log('Proposals data for funding calculation:', proposalsResponse.data.map(p => ({ 
          id: p.proposalID, 
        ...
        const total = proposalsResponse.data.reduce((sum, proposal) => {
          ...
          let budget = 0;
          if (proposal.proposedBudget !== null && proposal.proposedBudget !== undefined) {
            if (typeof proposal.proposedBudget === 'string') {
              budget = parseFloat(proposal.proposedBudget) || 0;
            } else if (typeof proposal.proposedBudget === 'number') {
              budget = proposal.proposedBudget;
            }
          }
          console.log(`Proposal ${proposal.proposalID}: budget=${proposal.proposedBudget}, converted=${budget}`);
          return sum + budget;
        }, 0);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md text-center transition-transform duration-200 hover:-translate-y-1">
            <div className="text-4xl font-bold text-red-900 mb-3">
              {isNaN(totalFunding) || totalFunding === 0 ? '₱0.0M' : `₱${(totalFunding / 1000000).toFixed(1)}M`}
            </div>
            ...
            {filteredResearch.map((research, index) => (
              <div key={index} className="grid grid-cols-[2fr_1fr_1fr_1fr_120px] gap-4 p-4 hover:bg-gray-50 transition-colors duration-150">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600">ID: {research.displayId}</div>
