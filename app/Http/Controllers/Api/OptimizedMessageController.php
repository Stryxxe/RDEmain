...
class OptimizedMessageController extends Controller
{
    ...
    {
        $key = "msg_{$endpoint}_{$user->userID}_" . md5(serialize($params));
        return $key;
    }
    ...
    public function sent(Request $request): JsonResponse
    {
        ...
        $cacheKey = $this->getCacheKey($user, 'sent', ['per_page' => $perPage, 'page' => $page]);
    ...
    public function conversations(Request $request): JsonResponse
    {
        ...
        $cacheKey = $this->getCacheKey($user, 'conversations');
        
        $conversations = $this->deduplicateRequest($cacheKey, function () use ($user) {
            ...
            return $query->with(['sender.role', 'sender.department', 'recipient.role', 'recipient.department'])
                ->orderBy('created_at', 'desc')
                ->get()
    ...
    public function conversation(Request $request, $otherUserId): JsonResponse
    {
        ...
        $cacheKey = $this->getCacheKey($user, 'conversation', ['other_user' => $otherUserId]);
        
        $messages = $this->deduplicateRequest($cacheKey, function () use ($user, $otherUserId) {
            ...
            return $messages;
        }, 5); // Very short cache for active conversations
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $this->clearUserCaches($recipient);
    ...
    public function markAllAsRead(Request $request): JsonResponse
    {
        ...
        // Clear caches after marking all as read
        $this->clearUserCaches($user);
